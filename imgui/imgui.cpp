...
//	jelo add start
static ImGuiTextEditState gInputTextState ;
//	jelo add end
...
{
    const int n = ImStrlenW(gInputCharacters);
    if (n + 1 < IM_ARRAYSIZE(gInputCharacters))
    {
        gInputCharacters[n] = c;
        gInputCharacters[n+1] = '\0';
    }
}

void ImGuiIO::AddInputCharactersUTF8(const char* utf8_chars)
{
    // We can't pass more wchars than ImGuiIO::gInputCharacters[] can hold so don't convert more
    const int wchars_buf_len = sizeof(gInputCharacters) / sizeof(ImWchar);
...
static void SetCurrentWindow(ImGuiWindow* window)
{
    ImGuiState& g = (*(ImGuiState *)ImGui::GetCurrentGuiState ()) ;
...
void ImGui::NewFrame()
{
	...
	//	jelo add start
	SetCurrentFont(g.IO.Fonts->Fonts[1]);
    ...
    g.IO.WantTextInput = (g.ActiveId != 0 && gInputTextState.Id == g.ActiveId);
...
void ImGui::Shutdown()
{
    ...
    gInputTextState.Text.clear();
    gInputTextState.InitialText.clear();
    gInputTextState.TempTextBuffer.clear();
...
void ImGui::EndFrame()
{
    ...
    memset(gInputCharacters, 0, sizeof(gInputCharacters));
...
//	jelo add start
ImGuiWindow* FindWindowByNameJelo(const char* name)
{
    // FIXME-OPT: Store sorted hashes -> pointers.
    ImGuiState& g = (*(ImGuiState *)ImGui::GetCurrentGuiState ()) ;
    ImGuiID id = ImHash(name, 0);
...
// If returning 'true' the node is open and the user is responsible for calling TreePop
bool ImGui::TreeNodeVJelo(const void* ptr_id,bool open, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGui::PushID(ptr_id);
    const bool opened = ImGui::CollapsingHeader(g.TempBuffer, "", false,open);
    ImGui::PopID();
...
bool ImGui::TreeNodeJelo(const void* ptr_id,bool open, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    bool s = TreeNodeVJelo(ptr_id, open,fmt, args);
    va_end(args);
    return s;
}

// FIXME: Split into CollapsingHeader(label, default_open?) and TreeNodeBehavior(label), obsolete the 4 parameters function.
bool ImGui::CollapsingHeaderJelo(const char* label, const char* str_id, bool display_frame, bool default_open,bool collapsingOpen )
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    ImGuiTextEditState& edit_state = gInputTextState;
    ...
    if (g.ActiveId == id)
    {
        ...
        if (gInputCharacters[0])
        {
            // Process text input (before we check for Return because using some IME will effectively send a Return?)
            // We ignore CTRL inputs, but need to allow CTRL+ALT as some keyboards (e.g. German) use AltGR - which is Alt+Ctrl - to input certain characters.
            if (!(is_ctrl_down && !is_alt_down) && is_editable)
            {
                for (int n = 0; n < IM_ARRAYSIZE(gInputCharacters) && gInputCharacters[n]; n++)
                    if (unsigned int c = (unsigned int)gInputCharacters[n])
                    {
    ...
    if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
    {
        ...
        bool cursor_is_visible = (gInputTextState.CursorAnim <= 0.0f) || fmodf(gInputTextState.CursorAnim, 1.20f) <= 0.80f;
...
void ImGui::ShowMetricsWindow(bool* opened)
{
    if (ImGui::Begin("ImGui Metrics", opened))
    {
        ...
        ImGuiState& g = (*(ImGuiState *)ImGui::GetCurrentGuiState ()) ;                // Access private state
