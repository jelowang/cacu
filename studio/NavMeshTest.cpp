...
static H3NavMesh gNavMesh ;
static float* gNavMeshPath = 0;
static iS32 gNavMeshPathLength = 0;
...
NavMeshTest::NavMeshTest ( ){
	...
	gObject->addComponent ( MMC_NEW H3MeshRender ( MMC_NEW H3Mesh ( "models/path.obj", &material ) ) );
	gObject->setPosition ( H3Vector3f ( 0.0f, 0.0f, 0.0f ) );
	gObject->setBasicsAlignEnable ( false );
	addChild ( gObject );
	...
	//gNavMesh.load ( "models/111.bin" ) ;
	//gNavMesh.generatePath ( H3Vector3f ( 0, .0, 49.0 ), H3Vector3f ( 19.0, .0, -10.0 ) );
	gNavMesh.load ( "models/path.bin" );
	gNavMesh.generatePath ( H3Vector3f ( -17, .0, 18.00 ), H3Vector3f ( 19.0, .0, -18.0 ) );
	gNavMesh.getPath ( &gNavMeshPath, &gNavMeshPathLength );


}
NavMeshTest::~NavMeshTest ( ){}

void NavMeshTest::update ( iF32 deltaT ) {
}

void NavMeshTest::draw ( iF32 deltaT ){
	//	draw row
	iF32 r = 0.0f;
	iS32 row = 0;
	...
	if ( gNavMeshPathLength ) {
		for ( int i = 0; i < gNavMeshPathLength-1; ) {

			H3Vector3f o = H3Vector3f ( gNavMeshPath[i * 3], gNavMeshPath[i * 3 + 1], gNavMeshPath[i * 3 + 2] );
			H3Vector3f d = H3Vector3f ( gNavMeshPath[(i + 1) * 3], gNavMeshPath[(i + 1) * 3 + 1] + 0.1f, gNavMeshPath[(i + 1) * 3 + 2] );
...
void NavMeshTest::resize ( iU32 width, iU32 height ){
	this->width = width;
	this->height = height;
...
void NavMeshTest::keyEvent ( H3KeyEvent k ){}

void NavMeshTest::mouseEvent ( H3MouseEvent e ){
...
void NavMeshTest::accelerometerEvent ( H3AccelerometerEvent e ){}
void NavMeshTest::loadObject ( const iS8* model, const iS8* mtl ){}