...
int cacuMainWindowWidth = 1920 ;
int cacuMainWindowHeight = 1080 ;
...
static void initGuiSytle ()
{
	...
	ImFont* font1 = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\msyh.ttf", 20.0f, NULL, io.Fonts->GetGlyphRangesChinese());
...
int main(int, char**)
{
	...
	oMgr.Init();
	oMgr.SetMainTitle(H3UtilGbk2Utf8("Cacu Particle Studio | 超算粒子").data());
	ImwWindowManager::GetInstance()->GetConfig().m_bTabUseImGuiColors = false ;

	CacuEditorWindow* cacuEditorWindow = MMC_NEW CacuEditorWindow () ;
	MyImwWindow* style = new MyImwWindow () ;

	oMgr.Dock ( cacuEditorWindow ) ;
	oMgr.Dock ( style ) ;
	while (1) {
		oMgr.Run();
	}
